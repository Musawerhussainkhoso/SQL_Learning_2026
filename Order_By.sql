...
SELECT COL_NAMES
FROM TABLE_NAME
ORDER BY COL_NAMES ASC OR DESC;
*/
...
-- 1. Sort customers by balance (highest to lowest)
SELECT full_name, city, account_type, balance
FROM customers
ORDER BY balance DESC;

-- 2. Sort customers alphabetically by city, then by name within each city
SELECT full_name, city, balance
FROM customers
ORDER BY city ASC, full_name ASC;

-- 3. Sort only "Savings" account holders by balance (lowest to highest)
